...
const UPDATE_ALL_PAIRS_IN_MATERIA = 'UPDAUPDATE_ALL_PAIRS_IN_MATERIATE_TOP_PAIRS'
const UPDATE_ALL_TOKENS_IN_MATERIA = 'UPDATE_ALL_TOKENS_IN_MATERIA'
...
function reducer(state, { type, payload }) {
  switch (type) {
    ...
    case UPDATE_ALL_PAIRS_IN_MATERIA: {
    ...
    case UPDATE_ALL_TOKENS_IN_MATERIA: {
...
export default function Provider({ children }) {
  ...
  const updateAllPairsInMateria = useCallback((allPairs) => {
    dispatch({
      type: UPDATE_ALL_PAIRS_IN_MATERIA,
      payload: {
        allPairs,
      },
    })
  ...
  const updateAllTokensInMateria = useCallback((allTokens) => {
    dispatch({
      type: UPDATE_ALL_TOKENS_IN_MATERIA,
      payload: {
        allTokens,
      },
    })
      ...
      value={useMemo(
        () => [
          state,
          {
            ...
            updateEthPrice,
            updateTopLps,
            updateAllPairsInMateria,
            updateAllTokensInMateria,
          },
        ],
        [
          state,
...
async function getGlobalData(ethPrice, oldEthPrice) {
  ...
  try {
    ...
    data = result.data.materiaFactories[0]
    ...
    oneDayData = oneDayResult.data.materiaFactories[0]

    let twoDayResult = await client.query({
      query: GLOBAL_DATA(twoDayBlock?.number),
      fetchPolicy: 'cache-first',
    })
    twoDayData = twoDayResult.data.materiaFactories[0]

    let oneWeekResult = await client.query({
      query: GLOBAL_DATA(oneWeekBlock?.number),
      fetchPolicy: 'cache-first',
    })
    const oneWeekData = oneWeekResult.data.materiaFactories[0]
...
const getChartData = async (oldestDateToFetch) => {
  ...
  try {
    while (!allFound) {
      ...
      skip += 1000
      data = data.concat(result.data.materiaDayDatas)
      if (result.data.materiaDayDatas.length < 1000) {
        allFound = true
      }
    }
...
/**
 * Loop through every pair on materia, used for search
 */
async function getAllPairsOnMateria() {
  try {
    let allFound = false
    let pairs = []
    let skipCount = 0
...
/**
 * Loop through every token on materia, used for search
 */
async function getAllTokensOnMateria() {
  try {
    let allFound = false
    let skipCount = 0
    let tokens = []
...
export function useGlobalData() {
  const [state, { update, updateAllPairsInMateria, updateAllTokensInMateria }] = useGlobalDataContext()
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let allPairs = await getAllPairsOnMateria()
      updateAllPairsInMateria(allPairs)

      let allTokens = await getAllTokensOnMateria()
      updateAllTokensInMateria(allTokens)
    }
    if (!data && ethPrice && oldEthPrice) {
      fetchData()
    }
  }, [ethPrice, oldEthPrice, update, data, updateAllPairsInMateria, updateAllTokensInMateria])
...
export function useAllPairsInMateria() {
...
export function useAllTokensInMateria() {
  const [state] = useGlobalDataContext()
