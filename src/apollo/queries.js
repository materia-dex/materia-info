...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "slavni96/airetam") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const PRICES_BY_BLOCK = (tokenAddress, blocks) => {
  ...
  queryString += blocks.map(
    block => `
      b${block.timestamp}: bundle(id:${BUNDLE_ID}, block: { number: ${block.number} }) { 
        ethPrice
      }
    `
  )
...
export const GLOBAL_CHART = gql`
  query materiaDayDatas($startTime: Int!, $skip: Int!) {
    materiaDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
